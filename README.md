...
- Global population has grown significantly since 1960
...
The World Bank CSV has metadata rows at top that need to be removed first:
...
The years (1960–2024) are spread across columns. We need to unpivot them:
...
World Bank data includes regional aggregates (not real countries). Remove them using a Custom Column:
...
4. Filter IsCountry column → keep only TRUE
...
6. Filter to show only: China, India, United States, Indonesia, Brazil

### Step 11 — Final Polish

1. Add title: Insert → Text Box → Type World Population Dashboard
...
1. Download Power BI Desktop for free from https://powerbi.microsoft.com/desktop
